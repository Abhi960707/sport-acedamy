...
export default function GameReport({ searchQuery }) {
  ...
  const handleEditClick = (game) => {
    ...
    const isCustom = game.gameName && !PREDEFINED_GAMES.includes(game.gameName);
    setEditGame({
      ...game,
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      const { isOtherSelected, ...updateData } = editGame;
      const res = await api.put(`/games/update/${editGame._id}`, updateData);
        ...
        {loading ? (
          ...
          <div className="flex flex-col">
            <div className="md:hidden print:hidden print:grid-cols-2 print:gap-4 space-y-2 print:space-y-0 p-3 print:p-0">
              {(isPrinting ? filteredTasks : paginatedTasks).map((game, index) => (
                <article
                  key={game._id}
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fadeIn" style={{ zIndex: 9999 }}>
          <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 max-w-lg w-full overflow-hidden print:overflow-visible print:border-none print:shadow-none animate-fade-in-up">
            <div className="px-6 py-5 bg-gray-50/75 border-b border-gray-100 flex justify-between items-center">
              ...
              <button type="button" onClick={() => setEditGame(null)} className="text-gray-400 hover:text-gray-600 text-lg font-bold focus:outline-none">&times;</button>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    value={editGame.isOtherSelected ? 'Other' : editGame.gameName}
                    onChange={(e) => {
                      const val = e.target.value;
                      if (val === 'Other') {
                        setEditGame({ ...editGame, isOtherSelected: true, gameName: '' });
                      } else {
                        setEditGame({ ...editGame, isOtherSelected: false, gameName: val });
                      }
                    }}
                    required
                  >
                  ...
                  {editGame.isOtherSelected && (
                    <input
                      ...
                      onChange={(e) => setEditGame({ ...editGame, gameName: e.target.value })}

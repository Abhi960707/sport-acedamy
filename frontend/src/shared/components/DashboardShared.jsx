...
import CoachDashboard from './CoachDashboardShared';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        const [playersRes, coachesRes, gamesRes, auditRes, superStatsRes] = await Promise.all([
          api.get('/players/report'),
          api.get('/coach/report'),
          api.get('/games/report'),
          api.get('/audit/report'),
          userRole === 'superadmin' ? api.get('/superadmin/dashboard-stats') : Promise.resolve(null)
        ]);
        ...
        let adminsCount = 0;
        if (superStatsRes && superStatsRes.data && superStatsRes.data.success) {
            adminsCount = superStatsRes.data.data.totalAdmins || 0;
        }
  ...
  return (
    <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-fade-in-up">
      ...
      {/* Welcome Banner */}
      <section className="bg-gradient-to-r from-blue-600 via-indigo-600 to-teal-600 rounded-3xl p-6 sm:p-8 text-white shadow-xl relative overflow-hidden print:overflow-visible print:border-none print:shadow-none">
      ...
      <section className="space-y-4">
        ...
        {loading ? (
          <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 ${userRole === 'superadmin' ? 'xl:grid-cols-6' : 'xl:grid-cols-5'} gap-5`}>
            {[...Array(userRole === 'superadmin' ? 6 : 5)].map((_, i) => (
              <div key={i} className="h-28 bg-white border border-gray-100 rounded-2xl animate-pulse" />
            ))}
          </div>
        ) : (
          <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 ${cards.length === 6 ? 'xl:grid-cols-6' : 'xl:grid-cols-5'} gap-5`}>
            {cards.map((card, i) => (
              <Link
                key={i}
                to={card.link}
          ...
          {loading ? (
            ...
            <div className="space-y-4 pt-1">
              ...
              <div className="space-y-1">
                ...
                <div className="w-full bg-gray-100 h-2.5 rounded-full overflow-hidden print:overflow-visible print:border-none print:shadow-none">
                  <div 
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <section className="lg:col-span-7 space-y-4">
          ...
          <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden print:overflow-visible print:border-none print:shadow-none p-5 space-y-4">
            {loading ? (

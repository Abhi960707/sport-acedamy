...
function CoachAdd() {
  ...
  const validate = () => {
    ...
    } else if (addCoach.sportSpecialization === 'Other' && !customSport.trim()) {
      tempErrors.sportSpecialization = 'Please enter custom sport specialization';
    }
  ...
  const coachSubmit = async (e) => {
    ...
    const payload = { ...addCoach };
    if (payload.sportSpecialization === 'Other') {
      payload.sportSpecialization = customSport.trim();
    }

    setLoading(true);
    try {
      const res = await api.post('/coach/add', payload);
      const result = res.data;
      if (result.success) {
        ...
        <form className="p-6 sm:p-8 space-y-6" onSubmit={coachSubmit} id="coach-add-form" noValidate>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="space-y-1">
              ...
              {addCoach.sportSpecialization === 'Other' && (
                <input
                  type="text"
                  ...
                  onChange={(e) => {
                    ...
                    if (errors.sportSpecialization) {
                      setErrors(prev => ({ ...prev, sportSpecialization: '' }));
                    }
                  }}
                  className="w-full mt-2 px-4 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                />
              )}
            </div>

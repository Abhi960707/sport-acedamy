...
export default function Navbar({ searchQuery = '', setSearchQuery = () => { } }) {
  ...
  if (userRole === 'superadmin') {
    filteredNavLinks.push({ to: '/admin-management', label: 'Admins', icon: <FiUserCheck /> });
  }

  const finalNavLinks = filteredNavLinks.map(link => {
  ...
  if (userRole === 'superadmin') {
    filteredReportLinks.push({ to: '/academy-management', label: 'Academy Reports', icon: <FiFolder /> });
    filteredReportLinks.push({ to: '/login-log', label: 'Login Log', icon: <FiActivity /> });
  }
  ...
  useEffect(() => {
    ...
    const fetchSearchData = async () => {
      ...
      try {
        const [gRes, cRes, pRes, payRes, attRes, notifRes, admRes, acadRes] = await Promise.all([
          fetch(`${API_BASE}/games/report`, { headers: { Authorization: `Bearer ${token}` }, signal: controller.signal, cache: 'no-store' }),
          fetch(`${API_BASE}/coach/report`, { headers: { Authorization: `Bearer ${token}` }, signal: controller.signal, cache: 'no-store' }),
          fetch(`${API_BASE}/players/report`, { headers: { Authorization: `Bearer ${token}` }, signal: controller.signal, cache: 'no-store' }),
          fetch(`${API_BASE}/payments/report`, { headers: { Authorization: `Bearer ${token}` }, signal: controller.signal, cache: 'no-store' }),
          fetch(`${API_BASE}/attendance/report`, { headers: { Authorization: `Bearer ${token}` }, signal: controller.signal, cache: 'no-store' }),
          fetch(`${API_BASE}/notifications`, { headers: { Authorization: `Bearer ${token}` }, signal: controller.signal, cache: 'no-store' }),
          userRole === 'superadmin' ? fetch(`${API_BASE}/superadmin/admins`, { headers: { Authorization: `Bearer ${token}` }, signal: controller.signal, cache: 'no-store' }) : Promise.resolve(null),
          userRole === 'superadmin' ? fetch(`${API_BASE}/superadmin/academies`, { headers: { Authorization: `Bearer ${token}` }, signal: controller.signal, cache: 'no-store' }) : Promise.resolve(null),
        ]);
        ...
        const admJson = admRes ? await admRes.json() : { success: false };
        const acadJson = acadRes ? await acadRes.json() : { success: false };
        ...
        if (admJson.success) setAdminsData(admJson.data || []);
        if (acadJson.success) setAcademiesData(acadJson.data || []);
  ...
  const searchResults = useMemo(() => {
    ...
    return [
      ...
      ...mapMatches(coachesData, '/reportcoachs', 'name', ['name', 'coachId', 'sportSpecialization', 'email', 'contact'], 'Coach'),
      ...mapMatches(playersData, '/reportplayers', 'fullName', ['fullName', 'playerId', 'sportChosen', 'email', 'contactNumber'], 'Player'),
      ...
      ...mapMatches(adminsData, '/admin-management', 'name', ['name', 'email', 'role'], 'Admin'),
      ...mapMatches(academiesData, '/academy-management', 'academyName', ['academyName', 'email', 'status'], 'Academy'),
    ].slice(0, 8);
  }, [debouncedSearch, dataLoaded, gamesData, coachesData, playersData, paymentsData, attendanceData, adminsData, academiesData]);
  ...
  return (
    <nav className="sticky top-0 z-50 w-full bg-white/90 backdrop-blur-md border-b border-gray-100 shadow-sm" role="navigation" aria-label="Main navigation">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between gap-3">
          ...
          <div className="hidden lg:flex items-center gap-0.5 xl:gap-1 flex-1 justify-center">
            {finalNavLinks.map(({ to, label, icon }) => (
              <NavLink
                key={to}
                to={to}
                onClick={() => setSearchQuery('')}
                className={({ isActive }) =>
                  `flex items-center gap-1 px-1.5 lg:px-2 py-1.5 rounded-xl text-xs xl:text-sm font-semibold transition-all duration-200 ${isActive ? 'bg-blue-50 text-blue-600 shadow-sm shadow-blue-500/10' : 'text-gray-600 hover:text-blue-600 hover:bg-gray-50'
                  }`
                }
              >
            ...
            <div className="relative" ref={reportMenuRef}>
              <button
                ...
                className={`flex items-center gap-1 px-1.5 lg:px-2 py-1.5 rounded-xl text-xs xl:text-sm font-semibold transition-all duration-200 focus:outline-none select-none cursor-pointer ${isReportPage ? 'bg-blue-50 text-blue-600 shadow-sm shadow-blue-500/10' : 'text-gray-600 hover:text-blue-600 hover:bg-gray-50'
                  }`}
              >
          ...
          {/* Search Box */}
          <div className="hidden lg:flex relative items-center min-w-[150px] max-w-[220px] xl:max-w-sm flex-1" ref={searchRef}>
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-gray-100 bg-white/95 backdrop-blur-md px-4 py-4 space-y-4">

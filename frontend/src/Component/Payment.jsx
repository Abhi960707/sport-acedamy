          ...
          <div className="p-4 sm:p-6 space-y-3 max-h-[720px] overflow-auto">
            {loading ? (
              ...
              filteredRecords.map((record) => (
                <article key={record._id} className="rounded-2xl border border-gray-100 bg-gray-50/50 p-4 flex flex-col gap-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h4 className="font-bold text-gray-800">{record.playerName}</h4>
                      <p className="text-xs text-gray-500">{record.paymentDate}</p>
                    </div>
                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-bold text-green-700 bg-green-100">
                      ₹{record.amount}
                    </span>
                  </div>

                  <div className="flex flex-wrap gap-2 text-xs text-gray-500">
                    <span className="px-2 py-1 rounded-full bg-white border border-gray-200 capitalize">Method: {record.paymentMethod.replace('_', ' ')}</span>
                    {record.transactionId && <span className="px-2 py-1 rounded-full bg-white border border-gray-200">Txn: {record.transactionId}</span>}
                  </div>
                  ...
                  <div className="flex justify-end mt-1">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-blue-600 bg-blue-50 border border-blue-100 hover:bg-blue-600 hover:text-white rounded-xl transition-all"
                    >
                      <FiPrinter />
                      Print Receipt
                    </button>

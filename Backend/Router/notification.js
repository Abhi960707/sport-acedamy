...
router.get('/notifications', auth, async (req, res) => {
    try {
        const playerFilter = req.userRole === 'superadmin' ? {} : { owner: req.academyOwnerId };
        if (req.userRole === 'coach' && req.coachProfile) {
            playerFilter.coachAssigned = req.coachProfile.name;
        }
        ...
        let displaySaved = savedNotifications.filter(n => !n.isRead && !(n.title.startsWith('fee-') || n.title.startsWith('reg-') || n.title.startsWith('coach-')));

        if (req.userRole === 'coach' && req.coachProfile) {
            displaySaved = displaySaved.filter(n => {
                if (n.title.startsWith('Fee Reminder: ')) {
                    return n.title === `Fee Reminder: ${req.coachProfile.name}`;
                }
                return true;
            });
        } else if (req.userRole === 'admin') {
            displaySaved = displaySaved.filter(n => !n.title.startsWith('Fee Reminder: '));
        }
        ...
        res.status(500).json({ success: false, message: 'Failed to fetch notifications', error: process.env.NODE_ENV === 'production' ? 'Internal server error' : e.message });
    }
});
...
router.post('/notifications/notify-coaches', auth, async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Failed to notify coaches', error: process.env.NODE_ENV === 'production' ? 'Internal server error' : e.message });
    }
});
...
router.post('/notifications/read/:id', auth, async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Failed to mark read', error: process.env.NODE_ENV === 'production' ? 'Internal server error' : e.message });
    }
});
